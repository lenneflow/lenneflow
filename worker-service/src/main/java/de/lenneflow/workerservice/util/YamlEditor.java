...
public class YamlEditor {
    ...
    public static Deployment createKubernetesDeploymentResource(Function function, int replica, String serviceAccountName) {
        return new DeploymentBuilder().withApiVersion("apps/v1").withKind("Deployment")
                ...
                .withContainers().addNewContainer().withName(function.getName()).withImage(function.getImageName()).withPorts().addNewPort().withContainerPort(function.getServingPort())
    ...
    public static Service createKubernetesServiceResource(Function function, String serviceType) {
        return new ServiceBuilder().withApiVersion("v1").withKind("Service").withNewMetadata().withName(function.getName()).endMetadata().withNewSpec()
                .withType(serviceType).withSelector(Collections.singletonMap("app", function.getName())).withPorts().addNewPort().withPort(function.getAssignedHostPort()).withTargetPort(new IntOrString(function.getServingPort())).endPort()
                .endSpec().build();
    }
    ...
    public static Ingress createKubernetesIngressResource(Worker worker, Function function) {
        String ingressName = worker.getName().toLowerCase() + "-ingress";
